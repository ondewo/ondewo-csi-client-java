...
The published coordinates are `com.ondewo:ondewo-csi-client-java`. Maven:

```xml
<dependency>
  <groupId>com.ondewo</groupId>
  <artifactId>ondewo-csi-client-java</artifactId>
...
```groovy
dependencies { implementation 'com.ondewo:ondewo-csi-client-java:VERSION' }
```
...
> [search.maven.org](https://search.maven.org/artifact/com.ondewo/ondewo-csi-client-java).
...
dependencies { implementation 'com.github.ondewo:ondewo-csi-client-java:VERSION' }
```

### Jar from the GitHub release

Every [release](https://github.com/ondewo/ondewo-csi-client-java/releases) carries the built
`ondewo-csi-client-java-<version>.jar` and its sources jar as assets. Install one into
...
mvn install:install-file \
  -Dfile=ondewo-csi-client-java-<version>.jar \
  -DpomFile=pom.xml
```
...
```bash
git clone --recurse-submodules https://github.com/ondewo/ondewo-csi-client-java.git
cd ondewo-csi-client-java
...
service stubs of this client live in `ondewo.csi`. Each proto service `Foo` becomes a
`FooGrpc` class with a blocking, a future and an async stub; list what is available with:

```bash
ls src/main/java/ondewo/csi/*Grpc.java
```
...
// Replace <Service> with one of the services declared in the ondewo-csi-api protos,
// for example `ConversationsGrpc`.
...
outer class protoc names after the proto file (`conversation.proto` → `Conversation`, `agent.proto` → `AgentOuterClass` when
...
messages become top-level classes under `com/ondewo/nlu/`. Stubs therefore land under `ondewo.csi`, `ondewo.nlu`, `ondewo.s2t` and `ondewo.t2s`.

## Repository structure

```
.
├── ondewo-csi-api/               <----- submodule: the .proto definitions (input)
...
1. `checkout_defined_submodule_versions` — checks out the `ondewo-csi-api` and
...
```bash
docker run --rm \
  -v $(pwd)/ondewo-csi-api:/input-volume/ondewo-csi-api \
  -v $(pwd)/LICENSE:/input-volume/LICENSE \
  -v $(pwd):/output-volume \
  ondewo-java-proto-compiler \
  ondewo-csi-api ondewo com.ondewo ondewo-csi-client-java <version>
```
...
* **The output volume is the repository root**, which is precisely the maven project the image
  ...
  is the descriptor being re-rendered from `ONDEWO_CSI_VERSION`, and is intended.
...
`src/main/java/com/ondewo/csi/auth/`). Two rules follow from the pipeline:
...
| --- | --- |
| `com.ondewo.csi.stubs.GeneratedMessagesTest` | messages of both java flavours (the vendored nlu protos that set `java_multiple_files` → `com.ondewo.nlu`, everything else → an outer class in `ondewo.csi` / `ondewo.nlu` / `ondewo.s2t` / `ondewo.t2s`) survive a serialize/parse round trip, `optional` scalars keep explicit presence on the wire, enums keep their zero member, and the proto package in the descriptor is untouched by the `java_package` rewrite |
| `com.ondewo.csi.stubs.GeneratedServicesTest` | all 19 generated `*Grpc` classes (found on the compiled classpath, not listed by hand) expose a usable `ServiceDescriptor`; `ondewo.csi.Conversations` keeps its unary **and** its bidi-streaming rpcs; a unary call runs end to end over the in-process transport and the real generated marshallers; all three stub flavours build against a plain target channel |
| `com.ondewo.csi.auth.BearerTokenTest` | the hand-written `BearerToken` — validation, header shape, stub immutability |

Coverage is measured with **JaCoCo over the hand-written sources only**
(`com/ondewo/csi/auth/**`). Generated stubs are machine output and are excluded from the metric,
...
Versions are defined once, in the `Makefile`: `ONDEWO_CSI_VERSION` must match the
`ondewo-csi-api` release in major and minor version, and is written into the generated `pom.xml` by
the next `make build`.

1. Bump `ONDEWO_CSI_VERSION` and the submodule pins in the `Makefile`.
...
* [ONDEWO CSI API (protos)](https://github.com/ondewo/ondewo-csi-api)

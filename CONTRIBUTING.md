...
* **Almost nothing here is hand-written.** `src/main/java` and `pom.xml` are produced by the
  ...
  [ondewo-csi-api](https://github.com/ondewo/ondewo-csi-api) (the protos) or in
...
* **Hand-written classes live in their own java package** (for example
  `src/main/java/com/ondewo/csi/auth/`). The generator deletes only the leaf packages it
...
* **Versions are bumped in the `Makefile`**, never in `pom.xml`: `ONDEWO_CSI_VERSION`
  is written into the descriptor by the next `make build`, and must match the `ondewo-csi-api` release

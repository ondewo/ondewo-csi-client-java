...
# =====================================================================================
# ondewo-csi-client-java - Makefile
#
# The ONDEWO CSI (Conversational Speech Interface) gRPC client for java.
#
...
# protos of the ondewo-csi-api submodule.
#
# Quick start:
...
#
# Versioning: ONDEWO_CSI_VERSION (below) is the single source of truth. It is
...
MAVEN_ARTIFACT_ID=ondewo-csi-client-java
...
CURRENT_RELEASE_NOTES=`cat RELEASE.md \
	| perl -ne 'print if /Release ONDEWO CSI Java Client ${ONDEWO_CSI_VERSION}/../^\*{5}/'`

GH_REPO="https://github.com/ondewo/ondewo-csi-client-java"
...
install_precommit_hooks: ## Installs pre-commit hooks and sets them up for the ondewo-csi-client-java repo
...
TEST: ## Prints some important variables
	...
	@echo "Maven Artifact: \t ${MAVEN_GROUP_ID}:${MAVEN_ARTIFACT_ID}:${ONDEWO_CSI_VERSION}"
	@echo "Api Submodule: \t ${ONDEWO_API_DIR} @ ${ONDEWO_CSI_API_GIT_BRANCH}"
...
generate_ondewo_protos: ## Generate java gRPC stubs + the maven project from the proto files
	@test -d ${ONDEWO_PROTOS_DIR} || { echo "$(RED)[ERROR]$(NC) '${ONDEWO_PROTOS_DIR}' is missing - run 'make update_submodules' first"; exit 1; }
	@echo "$(BLUE)[INFO]$(NC) Generating ${MAVEN_GROUP_ID}:${MAVEN_ARTIFACT_ID}:${ONDEWO_CSI_VERSION} from ${ONDEWO_PROTOS_DIR} ..."
	docker run --rm \
...
checkout_defined_submodule_versions: ## Check out the submodule versions pinned above
	...
	git -C ${ONDEWO_API_DIR} checkout ${ONDEWO_CSI_API_GIT_BRANCH}
...
check_release_notes: ## Assert RELEASE.md carries an entry for ONDEWO_CSI_VERSION
...
# flip-flop greps for - is otherwise only noticed by whoever reads the release page afterwards.
	...
	if [ -z "$$notes" ]; then \
		echo "$(RED)[ERROR]$(NC) RELEASE.md has no 'Release ONDEWO CSI Java Client ${ONDEWO_CSI_VERSION}' entry"; \
		echo "        The GitHub release would be created with empty notes - add the entry first."; \
		exit 1; \
	fi; \
	echo "$(GREEN)[SUCCESS]$(NC) RELEASE.md has release notes for ${ONDEWO_CSI_VERSION}"
...
build_gh_release: check_release_notes ## Generate Github Release with CLI and attach the built jars
	...
	gh release create --repo $(GH_REPO) "$(ONDEWO_CSI_VERSION)" \
		-n "$(CURRENT_RELEASE_NOTES)" \
		-t "Release ${ONDEWO_CSI_VERSION}" \
		$(wildcard target/*.jar)
...
# release and is therefore never proven to be resolvable at all.
	...
	base=target/${MAVEN_ARTIFACT_ID}-${ONDEWO_CSI_VERSION}; \
...
# replaced or withdrawn - so the last step stays a deliberate human click.
	...
	@echo "$(GREEN)[SUCCESS]$(NC) uploaded ${MAVEN_GROUP_ID}:${MAVEN_ARTIFACT_ID}:${ONDEWO_CSI_VERSION}"
...
spc: ## Checks if the Release Branch, Tag and pom.xml version already exist
	...
	@if test "$(pom_version)" != "" && test "$(pom_version)" != "${ONDEWO_CSI_VERSION}"; then \
		echo "-- Test 3: pom.xml is at '$(pom_version)', not ${ONDEWO_CSI_VERSION} - run 'make build'!!"; exit 1; \
